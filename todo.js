...
/**
 * 커멘트를 입력받아 처리한다.
 * @param {'create' | 'read' | 'update' | 'delete'} command // 넷 중 하나
...
const updateTodo = (targetTodo) => {
  ...
  // }
  if (!todos.find((findID) => findID.id === targetTodo.id)) {
...
const deleteTodo = (id) => {
  ...
  // 여기에 삭제 로직을 작성하세요
  //ㅇㅇㅇ
  if (!todos.find((element) => element.id === id)) {
...
const test = () => {
  ...
  [1, 2, 10].map((id) => todoControl("delete", id)); // id = 10에 맞는 todo를 찾을 수 없습니다.
  [3, 4, 100].map((id) =>
